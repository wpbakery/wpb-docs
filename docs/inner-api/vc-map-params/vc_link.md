...
Link builder for creating links with URL, title, target, and rel attributes.
...
|-----------|------|-------------|
| `type` | String | **Required.** Must be `"vc_link"` |
...
```php
<?php
add_action('vc_before_init', 'my_element_with_vc_link');
function my_element_with_vc_link() {{
    vc_map(array(
