...
Special field for tab identification in tabbed elements.
...
|-----------|------|-------------|
| `type` | String | **Required.** Must be `"tab_id"` |
...
```php
<?php
add_action('vc_before_init', 'my_element_with_tab_id');
function my_element_with_tab_id() {{
    vc_map(array(
