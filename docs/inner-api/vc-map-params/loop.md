...
Query builder for constructing WordPress loop queries.
...
|-----------|------|-------------|
| `type` | String | **Required.** Must be `"loop"` |
...
```php
<?php
add_action('vc_before_init', 'my_element_with_loop');
function my_element_with_loop() {{
    vc_map(array(
