...
Custom options field for advanced configurations.
...
|-----------|------|-------------|
| `type` | String | **Required.** Must be `"options"` |
...
```php
<?php
add_action('vc_before_init', 'my_element_with_options');
function my_element_with_options() {{
    vc_map(array(
