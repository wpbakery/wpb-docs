...
Predefined parameter presets for quick configurations.
...
|-----------|------|-------------|
| `type` | String | **Required.** Must be `"params_preset"` |
...
```php
<?php
add_action('vc_before_init', 'my_element_with_params_preset');
function my_element_with_params_preset() {{
    vc_map(array(
