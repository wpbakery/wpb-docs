...
Sortable list of items that can be reordered.
...
|-----------|------|-------------|
| `type` | String | **Required.** Must be `"sorted_list"` |
...
```php
<?php
add_action('vc_before_init', 'my_element_with_sorted_list');
function my_element_with_sorted_list() {{
    vc_map(array(
