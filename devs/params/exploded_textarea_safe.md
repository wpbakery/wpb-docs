...
|-----------|------|-------------|
| `type` | String | **Required.** Must be `"exploded_textarea_safe"` |
...
```php
<?php
add_action('vc_before_init', 'my_element_with_exploded_textarea_safe');
function my_element_with_exploded_textarea_safe() {
    vc_map([
