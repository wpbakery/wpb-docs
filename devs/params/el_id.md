...
|-----------|------|-------------|
| `type` | String | **Required.** Must be `"el_id"` |
...
| &nbsp;&nbsp;&nbsp;&nbsp;`auto_generate` | Boolean | `false` | Whether to auto-generate the ID value |

## Complete Example

```php
<?php
add_action('vc_before_init', 'my_element_with_el_id');
function my_element_with_el_id() {
    vc_map([
