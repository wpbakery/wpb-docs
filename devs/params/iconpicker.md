...
|-----------|------|-------------|
| `type` | String | **Required.** Must be `"iconpicker"` |
...
| &nbsp;&nbsp;&nbsp;&nbsp;`emptyIcon` | Boolean | `false` | Whether an empty (no icon) option is available |
| &nbsp;&nbsp;&nbsp;&nbsp;`iconsPerPage` | Integer | - | Number of visible icons per page |
| &nbsp;&nbsp;&nbsp;&nbsp;`type` | String | - | Icon library name (e.g., `"fontawesome"`, `"openiconic"`, `"typicons"`, `"entypo"`, `"linecons"`, `"monosocial"`) |
| &nbsp;&nbsp;&nbsp;&nbsp;`source` | Array | - | Custom icon list for custom icon libraries |

## Complete Example

```php
<?php
add_action('vc_before_init', 'my_element_with_iconpicker');
function my_element_with_iconpicker() {
    vc_map([
