...
id: wpb_map
title: wpb_map()
sidebar_label: wpb_map()
sidebar_position: 11
slug: /inner-api/wpb_map
---

To add your shortcode to the WPBakery Page Builder content elements list, *wpb_map()* function should be called with an array of special attributes describing your shortcode.
...
| admin_enqueue_js | String\|Array | Absolute url to javascript file, this js will be loaded in the backend editor (it allows you to add more functionality to your shortcode in the backend editor mode). More info [here](/devs/developer-tutorials/enqueue-custom-element-assets) |
| admin_enqueue_css | String\|Array | Absolute url to css file, this css will be loaded in the backend editor (it allows you add custom css for element block to your shortcode in the backend editor mode). More info [here](/devs/developer-tutorials/enqueue-custom-element-assets) |
| front_enqueue_js | String\|Array | Absolute url to javascript file (useful for storing your custom backbone.js views), this js will be loaded in the plugin frontend editor mode (it allows you to add more functionality to your shortcode in the frontend editor mode). *(Available from plugin version 4.2.2)*. More info [here](/devs/developer-tutorials/enqueue-custom-element-assets) |
| front_enqueue_css | String\|Array | Absolute url to css file, this css will be loaded in the frontend editor (it allows you add custom css for element block to your shortcode in the frontend editor mode). *(Available from plugin version 4.2.2)*. More info [here](/devs/developer-tutorials/enqueue-custom-element-assets) |
| view_enqueue_js | String\|Array | Absolute url to javascript file, this js will be loaded on a view page. *(Available from plugin version 9.1)*. More info [here](/devs/developer-tutorials/enqueue-custom-element-assets) |
| view_enqueue_css | String\|Array | Absolute url to css file, this css will be loaded on a view page. *(Available from plugin version 9.1)*. More info [here](/devs/developer-tutorials/enqueue-custom-element-assets) |
| icon | String | URL or CSS class with icon image. More info [here](/devs/developer-tutorials/set-content-element-icon) |
| custom_markup | String | HTML template used to render the element in the **backend editor**. Supports Mustache-style placeholders. See [custom_markup template](#custom_markup-template) below |
...
`custom_markup` defines the HTML used to render an element inside the backend editor (the admin "block" view). When omitted, WPBakery falls back to a generic title + parameter holders block. The string is a Mustache-style template rendered against the element's attributes and inner content.

### Placeholders

| Placeholder | Description |
|---|---|
| `{{ params.<name> }}` | Escaped value of the `<name>` param. Use for attributes and text. |
| `{{{ params.<name> }}}` | **Unescaped** value of the `<name>` param. Use only for trusted HTML. |
| `{{ title }}` | Backend title bar generated by WPBakery for the element. |
| `{{ content }}` | Inner content (for container elements with `is_container => true`). |
| `{{ model_id }}` | Editor model id of the current element instance. |
| `{{ container-class }}` | Class hook injected by WPBakery for container drop zones. |

### Basic example

```php
wpb_map([
    "name"          => __("Notice", "domain"),
    "base"          => "my_notice",
    ...
    "params"        => [
        ...
        [
            ...
            "param_name" => "style",
            "value"      => [ "Info" => "info", "Warning" => "warning" ],
        ],
    ],
    "custom_markup" => '<div class="my-notice my-notice-{{ params.style }}"><strong>{{ params.title }}</strong></div>',
]);
```

### Container example

Container elements (`is_container => true` or `as_parent`) typically render `{{ content }}` so children appear inside the markup:

```php
wpb_map([
    "name"          => __("My Wrapper", "domain"),
    "base"          => "my_wrapper",
    "category"      => __("Content", "domain"),
    "is_container"  => true,
    "as_parent"     => [ "except" => "vc_row" ],
    "js_view"       => "VcColumnView",
    "params"        => [],
    "custom_markup" => '
<div class="my-wrapper {{container-class}}" data-vc-target-model-id="{{ model_id }}">
    {{ content }}
</div>',
]);
```
...
Attribute types can be extended with new custom types. Tutorial: [Create new param type](/devs/developer-tutorials/create-new-param-type)
...
If you are looking for a way to create content elements with "nested" structure, [have a look at this example](/devs/developer-tutorials/nested-shortcodes-container).
...
Now lets call wpb_map() function from our functions.php file with an array of special attributes to describe attributes of our shortcode. **Important:** *wpb_map()* call should be hooked on WPBakery Page Builder *vc_before_init* action.
...
Now update *wpb_map()* function call, and add new attribute to the params array. *wpb_map()* now should look like this:
